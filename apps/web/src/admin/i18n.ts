export const adminTr = (lang: 'zh' | 'en') => {
  const dict = {
    zh: {
      ...
      logout: '退出',
      back: '返回',
      nav: { overview: '总览', equipment: '设备', inquiry: '询盘', settings: '站点设置', mail: '邮件通知' },
...
contact_whatsapp: 'WhatsApp（号码或 wa.me 链接）', contact_telegram: 'Telegram（用户名或 t.me 链接）',
      ...
      mail: {
        title: '邮件通知',
        desc: '后台可配置 Resend 发信参数；密钥只写不读。修改保存后即时生效，无需重启。',
        source_db: '数据库',
        source_env: '环境变量',
        source_none: '未配置',
        ready_yes: '可发信',
        ...
        missing: '缺少：',
        field_enabled: '启用通知',
        field_enabled_hint_auto: '留空 = 有 Key 与收件人即视为开启',
        field_enabled_hint_on: '已强制开启',
        field_enabled_hint_off: '已强制关闭',
        clear_key: '清除已保存的 API Key',
        confirm_clear_key: '确认清除已保存的 API Key？清除后会回退到 Worker 环境变量（如有）。',
        key_cleared: '已清除，将回退到环境变量',
        field_from: '发件人（From）',
        field_from_hint: '形如 noreply@yourdomain.com 或 Minelink <noreply@yourdomain.com>',
        field_to: '收件人（To）',
        field_to_hint: '多人用「,」「;」或换行分隔，最多 10 个',
        field_cc: '抄送（Cc，可选）',
        field_reply_to: '回复地址（Reply-To，可选）',
        field_subject_prefix: '主题前缀',
        field_subject_prefix_hint: '询盘邮件主题前缀，便于在邮箱里过滤',
        field_api_key: 'Resend API Key',
        field_api_key_hint: '在 resend.com/api-keys 创建。留空表示清空（回退到 Worker 环境变量）。',
        field_api_key_set_db: '已保存（仅显示后 4 位）',
        field_api_key_env: '当前生效值来自 Worker 环境变量',
        save: '保存配置',
        saved: '已保存',
        save_failed: '保存失败',
        test_title: '发送测试邮件',
        test_to: '测试收件人（留空使用配置里的默认收件人）',
        test_send: '发送测试',
        test_result_sent: '已发送',
        test_result_failed: '失败',
        test_result_skipped: '跳过（查看配置）',
        test_result_err_required: '请先到 Resend 控制台用发件人域名验证一个发件地址，并把 Key 填到这里。',
        logs_title: '最近发送记录',
        logs_empty: '暂无发送记录',
        logs_col_type: '类型',
        logs_col_to: '收件人',
    ...
    en: {
      ...
      logout: 'Logout',
      back: 'Back',
      nav: { overview: 'Overview', equipment: 'Equipment', inquiry: 'Inquiries', settings: 'Site Settings', mail: 'Mail notifications' },
...
contact_whatsapp: 'WhatsApp (number or wa.me link)', contact_telegram: 'Telegram (username or t.me link)',
      ...
      mail: {
        ...
        desc: 'Configure Resend parameters from the admin. The API key is write-only and never echoed back. Changes apply instantly — no restart required.',
        source_db: 'Database',
        source_env: 'Environment variable',
        source_none: 'Not set',
        ready_yes: 'Ready to send',
        ready_no: 'Not ready',
        missing: 'Missing:',
        field_enabled: 'Enable notifications',
        field_from: 'From address',
        field_from_hint: 'e.g. noreply@yourdomain.com or Minelink <noreply@yourdomain.com>',
        field_to: 'To (recipients)',
        field_to_hint: 'Multiple addresses separated by "," ";" or newlines, up to 10',
        field_cc: 'Cc (optional)',
        field_reply_to: 'Reply-to (optional)',
        field_subject_prefix: 'Subject prefix',
        field_subject_prefix_hint: 'Prefix on every inquiry email subject; makes filtering easier in your inbox',
        field_api_key: 'Resend API Key',
        field_api_key_hint: 'Create one at resend.com/api-keys. Empty value clears the database copy (falls back to the Worker env var).',
        field_api_key_set_db: 'Saved (last 4 digits shown)',
        field_api_key_env: 'Effective value is the Worker env var',
        save: 'Save configuration',
        saved: 'Saved',
        save_failed: 'Save failed',
        test_title: 'Send a test email',
        test_to: 'Recipient (leave blank to use the default recipient)',
        test_send: 'Send test',
        test_result_sent: 'Sent',
        test_result_failed: 'Failed',
        test_result_skipped: 'Skipped (check configuration)',
        test_result_err_required: 'Verify your sending domain at resend.com, add a From address, then store the key here.',
        logs_title: 'Recent mail logs',
        logs_empty: 'No mail logs yet',
        logs_col_type: 'Type',
        logs_col_to: 'To',
        logs_col_subject: 'Subject',
        logs_col_status: 'Status',
        logs_col_time: 'Time',
        logs_col_error: 'Error',
      },
    },
  } as const;
  return dict[lang];
