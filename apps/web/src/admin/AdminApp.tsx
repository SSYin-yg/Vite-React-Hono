  ...
  return (
    <nav className={`admin-nav${open ? ' is-open' : ''}`} aria-label={t('title')}>
      <div className="admin-brand">
        ...
        <span><span className="admin-brand-name">Minelink</span><span className="admin-brand-sub">B2B ADMIN</span></span>
      </div>
      ...
      <NavLink to={`${base}/settings`} className={link} end>{ICONS.content}<span>{t('nav.settings')}</span></NavLink>
      ...
      <div className="admin-nav-foot">
        <div className="adm-profile">
          <span className="adm-profile-avatar">M</span>
          <span className="adm-profile-copy"><strong>{t('topbar.account')}</strong><small>Administrator</small></span>
        </div>
...
function useCrumbs(t:(k:string)=>string, base:string) {
  ...
  return useMemo(() => {
    ...
    const map: Record<string,string> = {
      overview: t('nav.overview'), equipment: t('nav.equipment'), inquiry: t('nav.inquiry'), settings: t('nav.settings'),
      new: t('topbar.new'), edit: t('topbar.edit'), import: t('topbar.import'),
    };
  ...
  return (
    <header className="adm-topbar">
      ...
      <nav className="adm-crumbs" aria-label="breadcrumb">
        {crumbs.map((c, i) => <span key={c.key} className="adm-crumb-item">{i > 0 && <span className="adm-crumb-sep">/</span>}<span className={i === crumbs.length - 1 ? 'adm-crumb-current' : 'adm-crumb-root'}>{c.label}</span></span>)}
      </nav>
      <div className="adm-top-actions">
        <span className="adm-online"><i /> Online</span>
        <button type="button" className="admin-btn admin-btn-icon" onClick={toggleTheme} title={t('topbar.theme')} aria-label={t('topbar.theme')}>{theme === 'dark' ? ICONS.sun : ICONS.moon}</button>
        <a className="admin-btn admin-btn-icon" href="/" target="_blank" rel="noreferrer" title={t('topbar.view_site')} aria-label={t('topbar.view_site')}>{ICONS.external}</a>
        <div className="adm-user" ref={menuRef}>
          <button type="button" className="adm-user-btn" onClick={() => setMenuOpen(v => !v)} aria-haspopup="menu" aria-expanded={menuOpen}>
            <span className="adm-avatar">M</span><span>{t('topbar.account')}</span><span className="adm-user-chevron">⌄</span>
          </button>
          {menuOpen && <div className="adm-menu" role="menu">
            <a className="adm-menu-item" href="/" target="_blank" rel="noreferrer">{ICONS.external}<span>{t('topbar.view_site')}</span></a>
            <div className="adm-menu-sep" />
            <button type="button" className="adm-menu-item" role="menuitem" onClick={() => { setMenuOpen(false); onLogout(); }}>{ICONS.logout}<span>{t('logout')}</span></button>
          </div>}
        </div>
      </div>
    </header>
...
function Login({ t, onOk }: { t:(k:string)=>string; onOk:()=>void }) {
  ...
  const submit = async (e:React.FormEvent) => {
    ...
    try { await adminLogin(token); setToken(''); onOk(); }
    catch (e2) { const ex = e2 as Error & { code?:string }; setErr(ex instanceof TypeError ? t('login.err_network') : ex.code === 'admin_not_configured' ? t('login.err_not_configured') : t('login.err_auth')); }
    finally { setBusy(false); }
  };
  return <div className="admin-login-wrap"><div className="admin-login">
    <div className="admin-login-brand"><span className="admin-brand-logo">{ICONS.equipment}</span><span className="admin-login-brand-name">Minelink</span></div>
    <div className="adm-login-eyebrow">B2B EQUIPMENT MANAGEMENT</div>
    <h2>{t('login.title')}</h2><p className="admin-login-sub">{t('login.placeholder')}</p>
    <form onSubmit={submit}><input type="password" value={token} onChange={e=>setToken(e.target.value)} aria-label={t('login.placeholder')} autoComplete="current-password"/><button type="submit" className="admin-btn admin-btn-primary" disabled={busy}>{busy?'…':t('login.submit')}</button></form>
    {err && <div className="admin-error" style={{marginTop:14}}>{err}</div>}<p className="admin-login-foot">{t('login.foot')}</p>
  </div></div>;
}

export default function AdminApp({ lang, base='/admin' }: { lang:Lang; base?:string }) {
  const dict = adminTr(lang); const t = (k:string) => resolve(dict, k);
  const [authed, setAuthed] = useState<boolean>(() => !!getAdminToken()); const [drawer, setDrawer] = useState(false); const { pathname } = useLocation();
  ...
  const shell = <div className="admin-shell">
    <AdminNav t={t} base={base} open={drawer} onLogout={logout}/>
    {drawer && <button className="adm-scrim" aria-label="close" onClick={() => setDrawer(false)}/>} 
    <div className="adm-body"><TopBar t={t} base={base} onBurger={() => setDrawer(v=>!v)} onLogout={logout}/>
      <main className="admin-main"><AdminCtx.Provider value={{lang, base, t}}>
        <Routes><Route index element={<Navigate to="overview" replace/>}/><Route path="overview" element={<Overview/>}/><Route path="equipment" element={<EquipmentList/>}/><Route path="equipment/new" element={<EquipmentForm/>}/><Route path="equipment/import" element={<EquipmentImport/>}/><Route path="equipment/:slug" element={<EquipmentForm/>}/><Route path="inquiry" element={<Inquiries/>}/><Route path="settings" element={<SiteSettings/>}/><Route path="*" element={<Navigate to="overview" replace/>}/></Routes>
      </AdminCtx.Provider></main>
    </div>
  </div>;
