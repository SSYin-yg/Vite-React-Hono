...
/** 把 `from / to / cc / reply_to / subject_prefix` 等表单状态集中维护 */
type Form = {
  enabled: '' | '1' | '0';
  from: string;
  to: string;
...
function formFrom(stored: MailConfigView['stored']): Form {
  return {
    enabled: stored.enabled === '1' || stored.enabled === 'true' ? '1' : stored.enabled === '0' || stored.enabled === 'false' ? '0' : '',
    from: stored.from,
    to: stored.to,
    cc: stored.cc,
    reply_to: stored.reply_to,
    subject_prefix: stored.subject_prefix,
    api_key: '', // 永远不回显——这就是「只写不读」的关键
  };
}
...
export default function MailSettings() {
  ...
  const loadLogs = async () => {
    setLogsLoading(true);
    try {
      const data = await listMailLogs(30);
      setLogs((data.items ?? []) as MailLog[]);
    } catch (e) {
      // 日志失败不影响主配置
      console.warn('mail logs failed:', e);
    } finally {
  ...
  const onSave = async () => {
    ...
    try {
      ...
      // enabled：只在与原值不同或显式设过才发
      if (form.enabled === '1') payload.enabled = '1';
      else if (form.enabled === '0') payload.enabled = '0';
      else payload.enabled = ''; // 留空=自动判断

      // 其它字段：未在 form 里动过的字符串后端会保持原值（PATCH 语义）
      if (form.from.trim()) payload.from = form.from.trim();
      else payload.from = '';
      if (form.to.trim()) payload.to = form.to.trim();
      else payload.to = '';
      if (form.cc.trim()) payload.cc = form.cc.trim();
      else payload.cc = '';
      if (form.reply_to.trim()) payload.reply_to = form.reply_to.trim();
  ...
  const onClearKey = async () => {
    ...
    setSaving(true);
    try {
      const next = await updateMailConfig({ api_key: '' });
      setCfg(next);
      setForm(formFrom(next.stored));
      toast.ok(t('mail.key_cleared'));
    } catch (e) {
      toast.err(String(e));
    } finally {
      setSaving(false);
  ...
  const onTest = async () => {
    ...
    try {
      ...
      else toast.warn(`${t('mail.test_result_failed')}：${r.error || r.status}`);
      loadLogs();
    } catch (e) {
      toast.err(String(e));
    } finally {
    ...
    <div className="mail-settings">
      <div className="inq-head">
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          {saved && <span className="admin-ok-inline">{dict.saved}</span>}
          <button className="admin-btn" onClick={() => { load(); loadLogs(); }} disabled={loading}>
        ...
        <>
          <div className={'mail-status ' + (cfg.ready ? 'is-ok' : 'is-warn')}>
            ...
            <div className="mail-status-sources">
              ...
              <span><SourceTag s={cfg.source.from} dict={dict} /> From</span>
              <span><SourceTag s={cfg.source.to} dict={dict} /> To</span>
            </div>
          </div>
