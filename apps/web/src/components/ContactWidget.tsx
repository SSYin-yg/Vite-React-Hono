...
const Ico = {
  launcher: (
    <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
      ...
      <circle cx="9" cy="12" r="1.3" /><circle cx="12" cy="12" r="1.3" /><circle cx="15" cy="12" r="1.3" />
    </svg>
  ),
  whatsapp: (
    <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
  ...
  telegram: (
    <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M11.94 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.06 0zm4.97 7.22c.1 0 .32.02.46.14a.51.51 0 0 1 .17.33c.02.09.04.31.02.48-.18 1.9-.96 6.5-1.36 8.63-.17.9-.5 1.2-.83 1.23-.7.07-1.24-.46-1.92-.91-1.07-.7-1.67-1.14-2.71-1.83-1.2-.78-.42-1.22.26-1.93.18-.18 3.25-2.97 3.31-3.23.01-.03.01-.15-.06-.21s-.17-.04-.25-.02c-.1.02-1.79 1.14-5.06 3.35-.48.33-.91.49-1.3.48-.43-.01-1.25-.24-1.87-.44-.75-.25-1.35-.38-1.3-.79.03-.22.32-.44.89-.67 3.48-1.52 5.78-2.52 6.92-3 .33-.14 1.18-.44 1.18-.44.05-.02.12-.03.18-.03z"/></svg>
  ),
  email: (
...
type Kind = 'whatsapp' | 'telegram' | 'email';
type SettingsLike = Record<string, unknown>;

function toHref(kind: Kind, raw: string): string | null {
  const v = raw.trim();
  if (!v) return null;
...
function toLabel(kind: Kind, raw: string): string {
  const v = raw.trim();
  if (!v) return '';
...
function enabled(settings: SettingsLike, key: string, fallback = true) {
  const value = settings[key];
  if (value === undefined || value === null || String(value).trim() === '') return fallback;
  return !/^(0|false|off|no)$/i.test(String(value).trim());
}

function orderedKinds(settings: SettingsLike): Kind[] {
  const fallback: Kind[] = ['whatsapp', 'telegram', 'email'];
  const raw = String(settings.contact_channels_order ?? '').trim();
  const parsed = raw.split(/[,;，；\s]+/).map((v) => v.trim().toLowerCase()).filter((v): v is Kind => fallback.includes(v as Kind));
...
export default function ContactWidget() {
  ...
  const txt = useCallback((key: string, fbZh: string, fbEn: string) => {
    const v = t(key);
    return v && v !== key ? v : (lang === 'zh' ? fbZh : fbEn);
  }, [t, lang]);
  ...
  const channels = useMemo(() => {
    const s = settings as SettingsLike;
    if (!enabled(s, 'contact_widget_enabled')) return [];
    const raw: Record<Kind, string> = {
      whatsapp: String(s.contact_whatsapp ?? '').trim(),
      telegram: String(s.contact_telegram ?? '').trim(),
      email: String(s.contact_email ?? '').trim(),
    };
    const enabledMap: Record<Kind, boolean> = {
      whatsapp: enabled(s, 'contact_whatsapp_enabled'),
      telegram: enabled(s, 'contact_telegram_enabled'),
      email: enabled(s, 'contact_email_enabled'),
    };
    return orderedKinds(s).map((kind) => {
      const href = enabledMap[kind] ? toHref(kind, raw[kind]) : null;
      if (!href) return null;
      const tip = txt(
        `contact.tip_${kind}`,
      ...
      return { kind, href, label: kind === 'whatsapp' ? 'WhatsApp' : kind === 'telegram' ? 'Telegram' : 'Email', value: toLabel(kind, raw[kind]), tip, external: kind !== 'email', icon: Ico[kind] };
    }).filter(Boolean) as { kind: Kind; href: string; label: string; value: string; tip: string; external: boolean; icon: ReactElement }[];
  }, [settings, txt]);
  ...
  useEffect(() => {
    ...
    if (!panel) return;
    const onStart = (e: TouchEvent) => { const p = e.changedTouches[0]; touch.current = { x: p.clientX, y: p.clientY, dragging: open }; };
    const onMove = (e: TouchEvent) => {
      ...
      if (dy > 0 && dy > dx) { e.preventDefault(); panel.style.transition = 'none'; panel.style.transform = `translateY(${Math.min(dy, 300)}px)`; }
    };
  ...
  const foot = txt('contact.foot', 'Henan Panshi Import and Export Trading Co., Ltd.', 'Henan Panshi Import and Export Trading Co., Ltd.');
  ...
  return (
    <div className={day ? 'cw-root cw-day' : 'cw-root cw-night'}>
      <div className="cw-pc-group" role="navigation" aria-label={title}>
        {channels.map((c) => (
          <a key={c.kind} className={`cw-btn cw-pc-btn cw-${c.kind}`} href={c.href} {...(c.external ? { target: '_blank', rel: 'noopener' } : {})} aria-label={c.tip}>
            {c.icon}<span className="cw-label">{c.tip}</span>
          </a>
        ))}
      </div>

      <button ref={launcherRef} type="button" className="cw-launcher" aria-label={openLabel} aria-expanded={open} aria-controls="cwPanel" onClick={() => setOpen(true)}>
        {Ico.launcher}
      </button>
      ...
      <div ref={panelRef} id="cwPanel" className={open ? 'cw-panel is-open' : 'cw-panel'} role="dialog" aria-modal="true" aria-labelledby="cwTitle" aria-hidden={!open}>
        ...
        <div className="cw-list" role="list">
          {channels.map((c) => (
            <a key={c.kind} className={`cw-item cw-${c.kind}`} href={c.href} role="listitem" {...(c.external ? { target: '_blank', rel: 'noopener' } : {})}>
              <span className="cw-icon">{c.icon}</span>
              <span className="cw-text"><strong>{c.label}</strong><small>{c.value}</small></span>
